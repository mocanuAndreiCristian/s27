...
function createId(prefix = "overlay") {
    return `${prefix}-${Date.now()}-${Math.random().toString(16).slice(2)}`;
}

function resolveElement(target) {
    if (!target || !window.document) return null;
    if (window.Element && target instanceof window.Element) return target;
    if (typeof target === "string") return window.document.getElementById(target);
    return null;
}
...
export class OverlayManager {
    constructor(options = {}) {
        ...
        this.body = window.document ? window.document.body : null;
        this.isReady = false;
    ...
    init() {
        if (this.isReady || !window.document) return;

        this.overlayBackdrop = window.document.getElementById("overlay");
        this.body = window.document.body || this.body;
    ...
    resolveRecord(id) {
        ...
        if (!record && window.document) {
            const element = window.document.getElementById(overlayId);
            if (element) {
                record = this.register(overlayId) || null;
            }
        }

        if (record && !record.element && window.document) {
            record.element = window.document.getElementById(overlayId);
        }
    ...
    close(id, options) {
        if (!id) return this.closeTop(options);
    ...
    closeTop(options) {
        ...
        return this.requestClose(topId, options && options.reason, options);
    }
...
export const overlayManager = window.overlayManager || new OverlayManager({
