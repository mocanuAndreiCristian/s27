(function () {
    ...
    function renderCardsScroll(w) {
        const hideEmpty = document.body.classList.contains('hide-empty-days');
        return `<div class="layout-cards-scroll-wrap">${days.map((d) => {
            const entries = w[d.k] || [];
            if (hideEmpty && (!entries || entries.length === 0)) return '';
            return panel(d, entries, "cards-scroll-day");
        }).join("")}</div>`;
    }
    function renderAccordion(w) {
        ...
        return `<div class="layout-accordion-wrap">${days
            .map((d, i) => {
                const en = w[d.k] || [];
                if (hideEmpty && en.length === 0) return '';
                const open = d.k === todayKey() || (!todayKey() && i === 0) ? " open" : "";
